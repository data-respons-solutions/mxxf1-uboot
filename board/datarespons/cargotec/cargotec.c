...
int board_late_init(void)
{
	...
	if (is_hab_enabled())
	{
		...
		setenv("bootscript", BOOTSCRIPT_SECURE);
		setenv("zimage", ZIMAGE_SECURE);
		setenv("initrd_file", "/boot/initrd-ivt_signed");
#ifdef CONFIG_BOOT_USB_INITRD
		setenv("bootscript_usb", BOOTSCRIPT_USB_SECURE);
#else
		setenv("bootscript_usb", BOOTSCRIPT_SECURE)
#endif
		//setenv("bootdelay", "0");
	}
	else
	{
		...
		setenv("bootscript", BOOTSCRIPT_NOSECURE);
		setenv("bootscript_usb", BOOTSCRIPT_NOSECURE);
	}
#else
	setenv("bootscript", BOOTSCRIPT_NOSECURE);
	setenv("bootscript_usb", BOOTSCRIPT_NOSECURE);
