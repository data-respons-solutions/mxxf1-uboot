...
int param_init(struct param *e, const char *from, int size)
{
    ...
    {
        fprintf(stderr, "%s: Internal error: Could not allocate memory. Aborting\n", __func__);
        return 255;
    }
...
int param_add(struct param *e, const char *param)
{
    ...
    //if (verbose)
    //    fprintf(stderr,"%s: Adding parameter '%s'\n", me, param);
...
int param_update(struct param *e, int index, const char *param)
{
    ...
    {
        fprintf(stderr, "%s: Internal error: Tried to update index out of range\n", __func__);
        return 1;
    }
    ...
    if (newl == 1)
    {
        fprintf(stderr, "%s: Internal error: Tried to update 0-length parameter\n",__func__);
        return 1;
    }
...
int param_find(struct param *e, const char *key)
{
    ...
    fprintf(stderr, "%s: Did not find any key '%s'\n", __func__,key);
    return -1;
}
...
int param_delete(struct param *e, int index)
{
    if (index<0 || index>=e->count)
    {
        fprintf(stderr, "%s: Internal error: Tried to delete index out of range\n", __func__);
        return 1;
    }
...
int param_parse(struct param *e)
{
    ...
    {
    	fprintf(stderr, "%s: CRC checksum correct (%08X)\n", __func__, crc);
    }
    ...
    fprintf(stderr, "%s: Parsed %d parameters\n", __func__, e->count);
...
int param_generate(struct param *e)
{
    ...
    while(i < count)
    {
        ...
        if (remain < len+1)
        {
            fprintf(stderr, "%s: No space left in nvram to write parameters\n", __func__);
            return -1;
        }
    ...
    fprintf(stderr, "%s: Calculated new checksum (%08X)\n", __func__, checksum);
