...
__maybe_unused static const char* hw_string[8] = {
	"REVC",
	"REVD",
	"REVE",
	"FUTURE",
...
int board_ehci_power(int port, int on)
{
	...
	case 1:
		if (on) {
			gpio_set_value(GPIO_USB_H1_EN, 1);
			mdelay(1000);
		}
		else {
			gpio_set_value(GPIO_USB_H1_EN, 0);
		}
		break;
	default:
...
int board_late_init(void)
{
	...
	case 0:
		...
		env_set("fdt_file", "/boot/linkbox2-revC.dtb");
		break;

	case 1:
		env_set("fdt_file", "/boot/linkbox2-revD.dtb");
		break;

	default:
		env_set("fdt_file", "/boot/linkbox2-revE.dtb");
		break;
	}
...
static struct mx6_mmdc_calibration mx6_mmcd_calib = {
	...
	.p0_mprddlctl =  0x44464A44,
	.p1_mprddlctl =  0x40404040,
	.p0_mpwrdlctl =  0x30302A30,
